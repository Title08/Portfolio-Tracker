            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 w-full max-w-lg shadow-2xl animate-in fade-in zoom-in-95 duration-200 transition-colors">
                <div className="p-6 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
                    <div className="flex items-center gap-2">
                        <div className="bg-emerald-500/10 p-2 rounded text-emerald-600 dark:text-emerald-500"><ArrowRightLeft size={20} /></div>
                        <h3 className="text-xl font-bold text-slate-900 dark:text-white">Currency Exchange</h3>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    <div className="space-y-1">
                        <label className="text-xs text-slate-500 dark:text-slate-400 uppercase font-semibold">From ({exchangeData.direction === 'THB_TO_USD' ? 'THB Source' : 'USD Source'})</label>
                        <select required name="sourceId" value={exchangeData.sourceId} onChange={handleExchangeChange} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg px-4 py-3 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1"><label className="text-xs text-slate-500 dark:text-slate-400 uppercase font-semibold">Amount ({exchangeData.direction === 'THB_TO_USD' ? 'THB' : 'USD'})</label><input required type="number" step="any" name="amount" value={exchangeData.amount} onChange={handleExchangeChange} placeholder="1000" className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg px-4 py-2 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500" /></div>
                        <div className="space-y-1"><label className="text-xs text-emerald-600 dark:text-emerald-400 uppercase font-semibold">Rate (THB/USD)</label><input required type="number" step="any" name="rate" value={exchangeData.rate} onChange={handleExchangeChange} placeholder="35.00" className="w-full bg-slate-50 dark:bg-slate-900 border border-emerald-500/30 dark:border-emerald-500/40 rounded-lg px-4 py-2 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500" /></div>
                    </div>
                    <div className="space-y-1">
                        <label className="text-xs text-slate-500 dark:text-slate-400 uppercase font-semibold">To ({exchangeData.direction === 'THB_TO_USD' ? 'USD Destination' : 'THB Destination'})</label>
                        <select name="destinationId" value={exchangeData.destinationId} onChange={handleExchangeChange} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg px-4 py-3 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500">
                        ...
                        {exchangeData.destinationId !== 'NEW' && exchangeData.direction === 'THB_TO_USD' && (<p className="text-[10px] text-emerald-600 dark:text-emerald-400 mt-1">Note: This will average the exchange rate of the existing wallet.</p>)}
                    </div>
