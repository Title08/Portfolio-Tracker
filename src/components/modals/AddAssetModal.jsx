        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-slate-800 rounded-2xl border border-slate-700 w-full max-w-lg shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                <div className="p-6 pb-0">
                    ...
                    <div className="grid grid-cols-3 gap-2 bg-slate-900/50 p-1 rounded-lg mb-6">
                        <button onClick={() => { setFormType('Investment'); setError(''); }} className={`py-2 text-sm font-medium rounded-md transition-all ${formType === 'Investment' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}>Investment</button>
                        <button onClick={() => { setFormType('USD'); setError(''); }} className={`py-2 text-sm font-medium rounded-md transition-all ${formType === 'USD' ? 'bg-blue-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}>USD Cash</button>
                        <button onClick={() => { setFormType('THB'); setError(''); }} className={`py-2 text-sm font-medium rounded-md transition-all ${formType === 'THB' ? 'bg-pink-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}>THB Cash</button>
                    </div>
                </div>
                        ...
                        <div className="space-y-1">
                            ...
                            {(newAsset.sector || newAsset.industry) && (
                                <div className="flex gap-2 mt-2 flex-wrap">
                                    {newAsset.sector && <span className="text-[10px] bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded border border-indigo-500/30">{newAsset.sector}</span>}
                                    {newAsset.industry && <span className="text-[10px] bg-purple-500/20 text-purple-300 px-2 py-0.5 rounded border border-purple-500/30">{newAsset.industry}</span>}
                                </div>
                            )}
                        </div>
                    )}
                    <div className="space-y-1"><label className="text-xs text-slate-400 uppercase font-semibold">Name</label><input required name="name" value={newAsset.name} onChange={handleInputChange} placeholder="Asset Name" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" /></div>
                    {formType === 'Investment' && (<div className="space-y-1"><label className="text-xs text-slate-400 uppercase font-semibold">Type</label><select name="type" value={newAsset.type} onChange={handleInputChange} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 text-white focus:ring-indigo-500 focus:outline-none"><option value="Stock">Stock</option><option value="Crypto">Crypto</option><option value="ETF">ETF</option></select></div>)}

                    {/* Investment Fields: Price, Quantity, Total Cost */}
                    {formType === 'Investment' ? (
                        <div className="space-y-3">
                            <div className="space-y-1">
                                <label className="text-xs text-slate-400 uppercase font-semibold">Price (USD)</label>
                                <input required type="number" step="any" name="price" value={newAsset.price} onChange={handleInputChange} placeholder="0.00" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    <label className="text-xs text-slate-400 uppercase font-semibold">Quantity</label>
                                    <input required type="number" step="any" name="quantity" value={newAsset.quantity} onChange={handleInputChange} placeholder="0.00" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-xs text-slate-400 uppercase font-semibold">Total Cost (USD)</label>
                                    <input required type="number" step="any" name="totalCost" value={newAsset.totalCost} onChange={handleInputChange} placeholder="0.00" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                                </div>
                            </div>
                        </div>
                    ) : (
                        ...
                        <div className="space-y-1">
                            <label className="text-xs text-slate-400 uppercase font-semibold">{formType === 'THB' ? 'Amount (THB)' : 'Quantity'}</label>
                            <input required type="number" step="any" name="quantity" value={newAsset.quantity} onChange={handleInputChange} placeholder="0.00" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                        </div>
                    )}
