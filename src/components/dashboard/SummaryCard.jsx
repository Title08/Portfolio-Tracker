    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                {/* Net Worth & Pie */}
                <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-3xl p-8 border border-slate-700 shadow-xl flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
                    <div className="z-10 relative">
                        <h2 className="text-slate-400 text-sm font-bold uppercase tracking-widest mb-2">Net Worth (Est.)</h2>
                        <div className="flex items-baseline gap-3 flex-wrap">
                            <span className="text-4xl md:text-5xl font-bold text-white tracking-tight">{formatCurrency(totalTHB, 'THB')}</span>
                            {dailyChangePercent !== 0 && (
                                <span className={`flex items-center text-sm font-bold px-2 py-1 rounded-full ${dailyChangePercent >= 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                    ...
                    <div className="h-40 w-40 md:h-48 md:w-48 flex-shrink-0 relative">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                ...
                                <RechartsTooltip contentStyle={{ backgroundColor: '#1e293b', borderColor: '#334155', borderRadius: '12px' }} formatter={(val) => formatCurrency(val, 'THB')} />
                            </PieChart>
                        </ResponsiveContainer>
                        {/* Center Text ? */}
                    </div>
            ...
            {stats && (
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700">
                        ...
                        {stats.bestAsset ? (
                            <>
                                <div className="text-2xl font-bold text-white truncate" title={stats.bestAsset.name}>{stats.bestAsset.name}</div>
                    ...
                    <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700">
                        ...
                        {stats.worstAsset ? (
                            <>
                                <div className="text-2xl font-bold text-white truncate" title={stats.worstAsset.name}>{stats.worstAsset.name}</div>
